...
class _ProductRegister extends State<ProductRegister>{
  ...
  final dio = Dio();
  final baseUrl = "http://192.168.40.40:8080";
  ...
  void onRegister() async{
    try{
      ...
      if(token == null ) {print("로그인 후 가능합니다"); return;} // 비로그인 상태
      // 2. 폼데이터 구성 (+ 첨부파일)
      FormData formData = FormData(); // 전송할 자료들을 바이너리(바이트) 타입으로 변경 // 이유 : 대용량
      // formData.fields.add(MapEntry("필드명", "값")); // 필드명은 자바의 DTO 필드명과 동일하게
      formData.fields.add(MapEntry("pname", pnameController.text)); // 폼에 입력받은 제품명 대입
      formData.fields.add(MapEntry("pcontent", pcontentController.text)); // 폼에 입력받은 제품내용 대입
      ...
      // 4. DIO 응답 처리
      if(reponse.statusCode == 201 && reponse.data == true) {
        print("제품등록 성공");
      }
    }catch(e) {
      print(e);
